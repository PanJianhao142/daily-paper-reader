...
RANKED_DIR = os.path.join(ROOT_DIR, "archive", "rank")
...
def load_config() -> Dict[str, Any]:
    ...
    try:
        ...
        print("[WARN] PyYAML not installed, skip config.yaml.")
        return {}
    try:
        ...
        print(f"[WARN] failed to read config.yaml: {exc}")
        return {}
...
def build_context_lists(config: Dict[str, Any], fallback_queries: List[Dict[str, Any]]) -> Tuple[List[str], List[str]]:
    ...
    return unique_list(keywords), unique_list(rewrites)
...
def call_filter(
    ...
    system_prompt = (
        "You are a research screening assistant with broad, creative thinking. "
        "Use the Keyword and the Rewritten Query to judge relevance and recommend papers. "
        "Return a JSON object with a 'results' array only."
    )
    user_prompt = (
        "Context (packaged):\n"
        f"Keywords: {json.dumps(keywords, ensure_ascii=False)}\n"
        f"RewrittenQueries: {json.dumps(rewrites, ensure_ascii=False)}\n\n"
        "Papers:\n"
        f"{json.dumps(docs, ensure_ascii=False)}\n\n"
        "Output JSON format example:\n"
        "{\"results\": [{\"id\": \"paper_id\", \"evidence\": \"short phrase\", \"score\": 7}]}\n\n"
        ...
        "Evidence should be a short paraphrased phrase (3-12 words) explaining relevance. "
        "Evidence does NOT need to be a direct quote. "
        "Then give a score (0-10). "
        "If unrelated, use evidence=\"not relevant\" and score 0."
    )
...
def process_file(
    ...
    if not papers or not queries:
        print("[WARN] missing papers or queries, skip.")
        return

    config = load_config()
    keywords, rewrites = build_context_lists(config, queries)
    ...
    candidate_ids = unique_list(candidate_ids)
    if not candidate_ids:
        print("[WARN] no candidates found with star_rating >= min_star.")
    ...
    if not docs:
        print("[WARN] candidate papers not found in paper map.")
        save_json(data, output_path)
        return
    ...
    print(
        ...
        f"| keywords={len(keywords)} rewrites={len(rewrites)}"
    )

    merged: Dict[str, Dict[str, Any]] = {}
    debug_dir = os.path.join(RANKED_DIR, "debug")
    for idx, batch in enumerate(batches, start=1):
        print(f"[INFO] filter batch {idx}/{len(batches)} docs={len(batch)}")
        try:
            results = call_filter(
                filter_client,
                keywords,
            ...
            print(f"[WARN] filter batch failed: {exc}")
            continue
        ...
        for item in results:
            ...
            if (prev is None) or (score > float(prev.get("score", 0))):
                merged[pid] = {"paper_id": pid, "score": score, "evidence": evidence}

    if not merged:
        print("[WARN] no llm results returned.")
        save_json(data, output_path)
        return
