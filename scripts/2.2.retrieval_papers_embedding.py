...
def rank_papers_for_queries(
  ...
  for q in queries:
    ...
    print(f"[INFO] 正在处理查询（{q.get('type')}）：tag={q.get('tag') or ''}")
...
def main() -> None:
  ...
  parser.add_argument(
    ...
    type=str,
    default=None,
    help="可选：当使用 --input 处理单个文件时，自定义输出 JSON 路径；批处理模式下将自动写入 archive/filtered/ 目录，默认后缀 .embedding.json。",
  )
  parser.add_argument(
    "--top-k",
    type=int,
  ...
  # 决定处理哪些输入文件：
  # - 如果指定了 --input，则只处理该文件；
  # - 否则遍历 archive/raw 目录下所有 .json 文件。
  if args.input:
    ...
    if args.output:
      ...
      # 单文件模式下，如未指定输出路径，则写入 archive/filtered，文件名与原始 JSON 保持一致
    ...
    for name in raw_files:
      ...
      # 批量模式下，输出文件名与原始文件名保持一致，但目录变为 archive/filtered
      base = name
      if base.lower().endswith(".json"):
        base = base[:-5]
      output_path = os.path.join(FILTERED_DIR, f"{base}.embedding.json")
