...
def process_file(
  ...
  if not papers_list or not queries:
    print(f"[WARN] 文件 {os.path.basename(input_path)} 中缺少 papers 或 queries，跳过。")
    return
  ...
  for q in queries:
    ...
    print(
      f"[INFO] Query tag={q.get('tag') or ''} | candidates={len(top_ids)} "
    ...
    for batch_idx, (batch_indices, batch_docs) in enumerate(batches, 1):
      print(
        ...
        f"| first_doc_len={len(batch_docs[0]) if batch_docs else 0}"
      )
    ...
    if not rrf_scores:
      print("[WARN] 本次 query 未得到有效 rerank 结果，跳过。")
      continue
