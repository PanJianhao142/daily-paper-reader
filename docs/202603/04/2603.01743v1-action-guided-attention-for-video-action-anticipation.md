...
tags: ["query:课题"]
score: 6.0
evidence: 利用预测动作序列进行视频动作预测
tldr: 针对视频动作预测中现有Transformer方法缺乏高层语义且易过拟合视觉线索的问题，本文提出动作引导注意力（AGA）机制。该方法利用预测的动作序列作为查询和键来引导序列建模，通过门控函数融合过去关键时刻与当前帧信息。在EPIC-Kitchens-100数据集上的实验证明，AGA具有良好的泛化能力和可解释性，能有效捕捉动作间的依赖关系并提供透明的预测见解。
motivation: 现有的动作预测方法过度依赖显式视觉线索，缺乏建模潜在意图所需的高层语义，导致模型容易过拟合且泛化能力受限。
method: 提出动作引导注意力（AGA）机制，利用预测的动作序列作为查询和键来引导注意力，并结合门控函数融合时空特征。
result: 在EPIC-Kitchens-100基准测试中，AGA在验证集和未见测试集上均展现出优异的泛化性能。
conclusion: AGA通过显式建模动作依赖关系，不仅提升了预测精度，还为模型的预测逻辑提供了高度的可解释性。
---

## 摘要
预测视频中的未来动作具有挑战性，因为观察到的视频帧仅提供了过去活动的证据，需要推断潜在意图才能预测即将发生的动作。现有的基于 Transformer 的方法依赖于像素表示上的点积注意力，往往缺乏对视频序列建模以进行有效动作预测所需的高层语义。因此，这些方法倾向于过拟合过去帧中存在的显式视觉线索，限制了它们捕捉潜在意图的能力，并降低了对未见样本的泛化性能。为了解决这一问题，我们提出了动作引导注意力（Action-Guided Attention, AGA），这是一种显式利用预测的动作序列作为查询（queries）和键（keys）来引导序列建模的注意力机制。我们的方法促使注意力模块根据即将发生的活动强调过去的相关时刻，并通过专门的门控函数将这些信息与当前帧的嵌入相结合。AGA 的设计支持对从训练集中发现的知识进行训练后分析。在广泛采用的 EPIC-Kitchens-100 基准测试上的实验表明，AGA 从验证集到未见测试集都具有良好的泛化能力。训练后分析可以进一步检查模型捕获的动作依赖关系及其内化的反事实证据，为其预测性预报提供透明且可解释的见解。
