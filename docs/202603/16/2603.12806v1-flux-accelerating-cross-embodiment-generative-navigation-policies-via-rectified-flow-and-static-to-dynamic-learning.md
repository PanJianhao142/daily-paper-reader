...
score: 6.0
evidence: 用于导航策略的修正流
tldr: 本研究针对自主导航评估碎片化的问题，推出了包含物理真实人群模拟的动态导航基准DynBench。同时提出了FLUX，这是首个基于流匹配的统一导航策略。FLUX通过线性化概率流显著提升了推理效率，并采用从静态到动态的学习课程，先建立几何先验再通过强化学习优化社交避障能力。该模型在多项任务中达到SOTA，并成功在轮式、四足及人形机器人上实现零样本跨具身迁移。
motivation: 现有的自主导航评估协议碎片化，且缺乏能同时处理静态目标到达和动态社交避障的统一高效策略。
method: 提出FLUX策略，利用修正流线性化轨迹以提升推理速度，并结合静态几何先验与动态强化学习的课程学习方案。
result: "FLUX在推理效率上比扩散模型提升29%，在所有任务中取得最优性能，并实现了跨多种机器人形态的零样本实机迁移。"
conclusion: FLUX证明了流匹配模型在高效、通用的跨具身导航任务中的巨大潜力，为复杂动态环境下的机器人导航提供了新方案。
---

## 摘要
自主导航需要广泛的技能，从静态目标到达（goal-reaching）到动态社交穿梭（social traversal），然而评估在不同的协议之间仍然是碎片化的。我们引入了 DynBench，这是一个具有物理真实人群模拟特征的动态导航基准。结合现有的静态协议，它支持对六项基础导航任务的全面评估。在此框架内，我们提出了 FLUX，这是首个基于流（flow-based）的统一导航策略。通过线性化概率流，FLUX 用直线轨迹取代了迭代去噪，相比之前的基于流的方法，单步推理效率提高了 47%，相比基于扩散的方法提高了 29%。遵循从静态到动态的课程学习方案，FLUX 最初建立几何先验，随后通过在动态社交环境中的强化学习进行优化。这种方案不仅加强了社交感知导航，还通过随机动作分布捕捉恢复行为，从而增强了静态任务的鲁棒性。FLUX 在所有任务中均达到了最先进的性能，并在轮式、四足和人形平台上展示了无需任何微调的零样本从模拟到现实（sim-to-real）的迁移。
