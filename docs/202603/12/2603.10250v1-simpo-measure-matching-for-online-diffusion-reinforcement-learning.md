...
score: 6.0
evidence: 通过重加权匹配和扩散策略引导流模型
tldr: 针对扩散强化学习中 Softmax 重加权导致的过度贪婪及无法利用负样本反馈的问题，本文提出 SiMPO 框架。该框架通过两阶段测度匹配，将重加权方案推广至任意单调函数，并引入“符号测度”以支持负权重。SiMPO 不仅能主动排斥次优动作，还为不同奖励场景提供了灵活的加权指导，在多项任务中显著提升了在线扩散 RL 的性能。
motivation: 现有的扩散 RL 算法主要依赖 Softmax 重加权，难以有效利用负反馈且容易导致策略过度贪婪。
method: 提出 SiMPO 框架，通过 f-散度正则化构建允许负值的虚拟目标测度，并利用该符号测度指导扩散模型进行重加权匹配。
result: 实验证明 SiMPO 在性能上优于现有方法，并展示了负重加权在排斥次优动作方面的几何优势。
conclusion: SiMPO 为扩散 RL 提供了一个统一且灵活的理论框架，证明了引入负权重和多样化加权函数对提升策略性能的重要性。
---

## 摘要
一类常用的扩散策略强化学习（RL）算法对行为策略进行 softmax 重加权，这通常会导致过度贪婪的策略，且无法利用负样本的反馈。在这项工作中，我们引入了有符号测度策略优化（Signed Measure Policy Optimization, SiMPO），这是一个简单且统一的框架，利用通用的单调函数推广了扩散强化学习中的重加权方案。SiMPO 通过两阶段测度匹配的视角重新审视了扩散强化学习。首先，我们通过 $f$-散度正则化的策略优化构建一个虚拟目标策略，其中我们可以放宽非负性约束，以允许有符号的目标测度。其次，我们利用该有符号测度通过重加权匹配来引导扩散或流模型。这种表述具有两个关键优势：a) 它推广到了任意单调递增的加权函数；b) 它为负重加权（negative reweighting）提供了理论依据和实践指导。此外，我们提供了几何解释，以说明负重加权如何主动使策略远离次优动作。广泛的实证评估表明，SiMPO 通过利用这些灵活的加权方案实现了卓越的性能，并且我们为根据奖励景观（reward landscape）选择定制的重加权方法提供了实践指南。
