...
title_zh: SceMoS：通过几何关联 Token 进行规划的场景感知 3D 人体运动合成
...
tags: ["query:课题"]
score: 7.0
evidence: 具有全局规划和局部执行的场景感知3D人体运动合成
tldr: SceMoS 是一种场景感知的 3D 人体运动合成框架，旨在解决现有方法依赖昂贵 3D 数据且难以平衡规划与物理约束的问题。它通过将全局规划与局部执行解耦，利用 2D 鸟瞰图（BEV）进行全局路径规划，并结合局部高度图训练几何感知的运动分词器。该方法在 TRUMANS 基准上实现了 SOTA 的真实感和接触准确性，同时显著降低了参数量。
motivation: 现有的 3D 场景运动合成方法依赖高成本的 3D 体素或点云数据，且难以同时兼顾高层语义规划与底层物理可行性。
method: 提出 SceMoS 框架，利用 BEV 图像进行全局运动规划，并通过基于局部高度图的条件 VQ-VAE 训练几何感知的运动分词器以确保物理一致性。
result: "在 TRUMANS 基准测试中，该方法在运动真实感和接触准确度上达到领先水平，且场景编码参数量减少了 50% 以上。"
conclusion: 研究证明结构化的 2D 场景表示可以作为 3D 监督的有效替代方案，在降低计算开销的同时实现高质量的 3D 人机交互合成。
---

## 摘要
在真实场景中合成文本驱动的 3D 人体运动需要同时学习语义意图（例如“走向沙发”）和物理可行性（例如避免碰撞）。当前的方法通常采用生成式框架，同时学习高层规划和底层接触推理，并依赖于计算成本昂贵的 3D 场景数据，如点云或体素占用网格。我们提出了 SceMoS，这是一个场景感知运动合成框架，它证明了结构化 2D 场景表示可以作为物理关联运动合成中完整 3D 监督的强大替代方案。SceMoS 利用轻量级 2D 线索将全局规划与局部执行解耦，其核心依赖于：(1) 一个以文本为条件的自回归全局运动规划器，该规划器以从场景高处渲染并使用 DINOv2 特征编码的鸟瞰图 (BEV) 图像作为场景表示；(2) 一个通过条件 VQ-VAE 训练的几何关联运动分词器 (tokenizer)，它利用 2D 局部场景高度图将表面物理特性直接嵌入到离散词汇表中。这种 2D 分解实现了效率与保真度之间的权衡：BEV 语义捕捉空间布局和可供性 (affordance) 以进行全局推理，而局部高度图则在无需完整 3D 体积推理的情况下确保了细粒度的物理遵循。SceMoS 在 TRUMANS 基准测试中实现了最先进的运动逼真度和接触准确度，同时将场景编码的可训练参数减少了 50% 以上，表明 2D 场景线索可以有效地为 3D 人体-场景交互提供物理支撑。
